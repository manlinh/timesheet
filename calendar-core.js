...
let currentMonth = 6;  // 7 月，JS中從 0 開始
...
function renderCalendar() {
  ...
  document.getElementById("calendar-title").innerText = `📅 ${year} 年 ${month + 1} 月行事曆`;
  ...
  for (let date = 1; date <= daysInMonth; date++) {
    ...
    html += `<td onclick="openPopup('${iso}')"><div class="date-label">${date}</div>${entries}<button onclick="event.stopPropagation(); openPopup('${iso}')">➕</button></td>`;
    dc++;
  ...
  document.getElementById("calendar-log").innerHTML =
    logData.slice().reverse().map(l =>
      `<div><b>${l.date}</b> - ${l.action} ${l.time ? `(${l.time})` : ""} ${l.subject || "（刪除）"} by ${l.user}
       <small>${new Date(l.timestamp * 1000).toLocaleString()}</small></div>`).join("");
}
